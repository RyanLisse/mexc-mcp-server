...
import { createSuccessResponse, retryWithBackoff, validateOrThrow } from "../shared/utils/index.js";
...
export const TickerDataSchema = z.object({
  ...
  high: z.string(),
  low: z.string(),
  count: z.number().nullable().transform(val => val ?? 0), // Handle null values
  timestamp: z.number(),
});
...
export const Stats24hDataSchema = z.object({
  symbol: z.string(),
  volume: z.string(),
  ...
  high: z.string(),
  low: z.string(),
  trades: z.number().nullable().transform(val => val ?? 0), // Handle null values
  timestamp: z.number(),
});
...
export async function executeGetTicker(input: unknown): Promise<MarketDataResponse<TickerData>> {
  ...
  try {
    ...
    // Validate response
    const validatedData = validateOrThrow(TickerDataSchema, data);
    ...
    throw new Error(`Failed to fetch ticker data: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export async function executeGetOrderBook(input: unknown): Promise<MarketDataResponse<OrderBookData>> {
  ...
  try {
    ...
    throw new Error(`Failed to fetch order book data: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export async function executeGet24hStats(input: unknown): Promise<MarketDataResponse<Stats24hData[]>> {
  ...
  try {
    ...
    // Validate response
    const validatedData = data.map(item => validateOrThrow(Stats24hDataSchema, item));
    ...
    throw new Error(`Failed to fetch 24h stats: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export async function executeTestConnectivity(input: unknown): Promise<MarketDataResponse<any>> {
  try {
...
export async function executeTestAuthentication(input: unknown): Promise<MarketDataResponse<any>> {
  try {
...
export async function executeGetActiveSymbols(input: unknown): Promise<MarketDataResponse<string[]>> {
  ...
  try {
    ...
    throw new Error(`Failed to fetch active symbols: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
