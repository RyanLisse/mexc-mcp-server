...
import type { RiskAssessment, OptimizationResult, AnalysisType } from '../types/ai-types';
import { createAIAnalysisError } from '../errors';
import { clamp, sanitizeStringArray, ensureValidConfidence } from './index';
...
export function isValidRiskAssessment(data: unknown): data is RiskAssessment {
  ...
  if (!responseData.riskLevel || 
      !['low', 'medium', 'high'].includes(responseData.riskLevel)) {
    return false;
  }
...
export function isValidOptimizationResult(data: unknown): data is OptimizationResult {
  ...
  // Validate required fields
  if (!responseData.optimizationType || 
  ...
  if (responseData.allocations && Array.isArray(responseData.allocations)) {
    for (const allocation of responseData.allocations) {
      ...
      if (typeof allocation.currentWeight !== 'number' || 
          allocation.currentWeight < 0 || allocation.currentWeight > 1) return false;
      if (typeof allocation.optimizedWeight !== 'number' || 
          allocation.optimizedWeight < 0 || allocation.optimizedWeight > 1) return false;
    }
  }
...
export function parseRiskAssessmentResponse(response: unknown): RiskAssessment {
  ...
  const canApplyFallback = response !== null && 
                          response !== undefined && 
                          typeof response === 'object' && 
                          !Array.isArray(response) &&
...
function createFallbackOptimizationResult(response: unknown): OptimizationResult {
  ...
  if (response && typeof response === 'object' && !Array.isArray(response)) {
    ...
    if (resp.data && typeof resp.data === 'object') {
      ...
      if (typeof data.optimizationType === 'string' && 
      ...
      if (data.backtestResults && typeof data.backtestResults === 'object') {
        ...
        if (typeof backtest.periodMonths === 'number' && backtest.periodMonths > 0) {
          fallbackData.data.backtestResults = {
            ...
            volatility: typeof backtest.volatility === 'number' ? Math.max(0, backtest.volatility) : 0,
