...
async function main() {
  ...
  try {
    ...
    console.log('   Status: ✅ PASS');
    console.log(
      `   Best Bid: $${orderBookResult.data.bids[0]?.[0]} (${orderBookResult.data.bids[0]?.[1]} BTC)`
    );
    console.log(
      `   Best Ask: $${orderBookResult.data.asks[0]?.[0]} (${orderBookResult.data.asks[0]?.[1]} BTC)`
    );
  ...
  try {
    ...
    for (const [checkName, checkResult] of Object.entries(health.checks)) {
      console.log(
        `   ${checkName}: ${checkResult.status === 'pass' ? '✅' : '❌'} ${checkResult.message}`
      );
    }
  } catch (error) {
    console.log('   Status: ❌ FAIL');
