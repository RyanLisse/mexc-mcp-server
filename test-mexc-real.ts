...
async function testMexcIntegration() {
  ...
  try {
    ...
    console.log(`- API Key: ${marketDataConfig.mexc.apiKey ? marketDataConfig.mexc.apiKey.substring(0, 8) + '...' : 'NOT SET'}`);
    console.log(`- Secret Key: ${marketDataConfig.mexc.secretKey ? '[HIDDEN - Length: ' + marketDataConfig.mexc.secretKey.length + ']' : 'NOT SET'}`);
